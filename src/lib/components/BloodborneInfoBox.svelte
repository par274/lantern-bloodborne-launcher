<script lang="ts">
	...
	function displayValue(value: string | null | undefined): string {
		return value?.trim() || $t('bloodborne.info.unavailable');
	}
			...
			<div class="relative z-10 flex items-start justify-between gap-5">
				<div class="min-w-0">
					<div class="text-[0.64rem] font-semibold uppercase tracking-[0.26em] text-[#ccb57a]/72">
						{$t('bloodborne.info.eyebrow')}
					</div>
					<h2 class="mt-2 text-[1.65rem] font-semibold leading-tight text-[#f5ecd5]">
						{$t('bloodborne.info.title')}
					</h2>
					<p class="mt-2 max-w-[420px] text-[0.82rem] leading-[1.7] text-white/52">
						{$t('bloodborne.info.description')}
					</p>
				</div>
